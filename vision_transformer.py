...
class Attention(nn.Module):
    ...
    def forward(self, x, t=None):
        ...
        if t is not None:
            ...
            # if self.training:
            #     # row_score = torch.mean(attn[:, :, :, 11:], dim=-1)#.detach()
            #     mask_matrix = torch.ones((N, N)).to(x.device)
            #     mask_random = (torch.rand((N, 10)).to(x.device) > 0.0).float()
            #     mask_matrix[:, 1 : 11] = mask_matrix[:, 1 : 11] - mask_random * 1.
            #     attn = attn * mask_matrix
            #     # attn = attn.masked_fill(mask_matrix == -1, -1e9)
            #     # select_column = (torch.rand(10).to(x.device) > 0.5).float()
            mask = torch.ones(N).to(x.device) + t
            mask[0] = 1
            mask[11:] =1
            # attn[:, :, :, 1:11] = attn[:, :, :, 1:11]  / t
            attn = attn * mask
            raw_attn = attn
            attn = attn.softmax(dim=-1)
...
class Block(nn.Module):
    ...
    def forward(self, x, t=None):
        ...
        tmp, raw_attn = self.attn(self.norm1(x), t=t)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.learnable_temperature_head = nn.Parameter(torch.randn(4), requires_grad=True)
        self.learnable_temperature_medium = nn.Parameter(torch.randn(4), requires_grad=True)
        self.learnable_temperature_few = nn.Parameter(torch.randn(4), requires_grad=True)
        nn.init.constant_(self.learnable_temperature_head, 0)
        nn.init.constant_(self.learnable_temperature_medium, 0)
        nn.init.constant_(self.learnable_temperature_few, 0)
    ...
    def forward_features(self, x, cls_features=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for ind in range(medium_depth, len(self.blocks)):
                ...
                new_x, attn = self.blocks[ind](cls_token3, t=self.learnable_temperature_few[ind-medium_depth])
                # new_x, attn = self.blocks[ind](cls_token3, t=1)
            ...
            for ind in range(medium_depth, len(self.blocks)):
                ...
                new_x, attn = self.blocks[ind](cls_token2, t=self.learnable_temperature_medium[ind-medium_depth])
            ...
            for ind in range(medium_depth, len(self.blocks)):
                ...
                new_x, attn = self.blocks[ind](cls_token1, t=self.learnable_temperature_head[ind-medium_depth])
                # new_x, attn = self.blocks[ind](cls_token1, t=1)
        ...
        res['reg'] = torch.mean(self.learnable_temperature_few) * 2 +\
                        torch.mean(self.learnable_temperature_medium) * 1.5 +\
                                 torch.mean(self.learnable_temperature_head) * 1
